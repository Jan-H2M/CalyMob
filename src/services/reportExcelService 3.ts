...
const LABEL_OVERRIDES: Record<string, string> = {
  // Cotisation labels: Firestore AccountCodeService heeft nu correcte labels
  // (overrides verwijderd — waren verschoven en veroorzaakten foutieve labels)
...
function addBalanceSheet(
  ...
  const writeValCell = (addr: string, codeStr: string, field: 'openingValue' | 'closingValue', donneesCol: 'C' | 'D') => {
  ...
  const writeActifVals = (row: number, codeStr: string) => {
    writeValCell(`E${row}`, codeStr, 'openingValue', 'C');
    writeValCell(`F${row}`, codeStr, 'closingValue', 'D');
  };
  const writePassifVals = (row: number, codeStr: string) => {
    writeValCell(`J${row}`, codeStr, 'openingValue', 'C');
    writeValCell(`K${row}`, codeStr, 'closingValue', 'D');
  };
  ...
  // Official shows '-' for opening, 0 for closing
  const oblDRow = getDonneesRow('02.04');
  const oblOpening = getVal('02.04', 'openingValue');
  const oblClosing = getVal('02.04', 'closingValue');
  if (oblDRow) {
    sheet.getCell('E20').value = { formula: `'Données'!C${oblDRow}`, result: oblOpening };
    sheet.getCell('F20').value = { formula: `'Données'!D${oblDRow}`, result: oblClosing };
  } else {
...
function addNRubriquesSheet(workbook: ExcelJS.Workbook): void {
  ...
  const N_RUBRIQUES_EXCLUDED = new Set(['612-00-624', '730-00-711']);
...
function addResultatsCourantSheet(
  ...
  const budgetMap = new Map<string, number>([
    ...
    ['730-00-610', -50],
    ['730-00-611', -7760],
    ['730-00-712', 12700],  // NB: officieel heeft 712=12700 (was 711 in oude code)
    ['730-00-713', 1600],
    ['730-00-714', 1600],
    ['611-00-616', -50],
    ['611-00-618', -200],
    ...
    ['612-00-622', -1500],
    ['612-00-623', -500],
    ['601-00-624', -1000],  // Officieel: 601-00-624, niet 612-00-624
    ['612-00-625', -300],
    ...
    ['15-000-772', 500],
    // Formation
    // (geen budget in officieel bestand voor formation codes)
  ]);
...
function addBudgetNextYearSheet(
  ...
  const writeBudgetRow = (
    ...
    row.values = [
      ...
      code,                                                          // K: Code right
      label,                                                         // L: Label right
      budget != null && budget !== 0 ? round2(budget) : null         // M: Budget right
    ];
    // Format numeric columns
  ...
  for (const gd of sortedGroups) {
    ...
    for (const acc of gd.accounts) {
      writeBudgetRow(currentRow, acc.code, acc.label, acc.ytd, acc.budget ?? 0);
      groupYTD = round2(groupYTD + acc.ytd);
      groupBudget = round2(groupBudget + (acc.budget ?? 0));
      currentRow++;

      // Inline provision rows after specific codes
      const provRows = provisionsByCode.get(acc.code);
      if (provRows) {
        for (const prov of provRows) {
          writeBudgetRow(currentRow, null, prov.label, prov.ytd, null);
    ...
    if (gd.group.id === 'soiree_annuelle') {
      ...
      writeBudgetRow(currentRow, null, `Variation d'inventaire (${year} - ${prevYear})`, variationInv, null);
  ...
  // BOTTOM SECTION: Cash Balance reconciliation
  if (openingBankBalance !== undefined) {
    writeBudgetRow(currentRow, null, 'Opening Cash Balance', round2(openingBankBalance), null);
    currentRow++;

    writeBudgetRow(currentRow, null, `Total Mouvement${year}`, totalYTD, null);
    currentRow++;

    writeBudgetRow(currentRow, null, 'Apport Cash Reserve', null, null);
    currentRow++;
    ...
    writeBudgetRow(currentRow, null, `Reprise provisions ${year}`, round2(-totalRepriseProvisions), null);
    provAdj += round2(-totalRepriseProvisions);
    currentRow++;

    if (piscineProvClosing !== 0) {
      writeBudgetRow(currentRow, null, `Provision pour Piscine ${year} a payer en ${year + 1}`, piscineProvClosing, null);
      provAdj += piscineProvClosing;
      currentRow++;
    }

    if (materielProvClosing !== 0) {
      writeBudgetRow(currentRow, null, `Provision pour achat de matériel en ${year}`, materielProvClosing, null);
      provAdj += materielProvClosing;
      currentRow++;
    }

    const mouvProv = round2(regulYTD);
    writeBudgetRow(currentRow, null, `Provision pour Mouvements ${year} affecté à ${year + 1}`, mouvProv, null);
    provAdj += mouvProv;
    currentRow++;

    // Balance Théorique
    const balanceTheorique = round2(openingBankBalance + totalYTD + provAdj);
    writeBudgetRow(currentRow, null, `Balance Theorique ${year}`, balanceTheorique, null);
    currentRow++;

    // Balance Banque
    if (closingBankBalance !== undefined) {
      writeBudgetRow(currentRow, null, 'Balance Banque', round2(closingBankBalance), null);
      currentRow++;

      // Difference
      const difference = round2(closingBankBalance - balanceTheorique);
      writeBudgetRow(currentRow, null, 'Difference', difference, null);
    }
  }
}
...
export async function generateCompteResultatsExcel(
  ...
  const writePLRow = (rowNum: number, code: string | null, label: string,
    ...
    const vals: (string | number | null)[] = [
      ...
      label,                                                    // C: label
      null,                                                     // D
      ytd != null && ytd !== 0 ? round2(ytd) : null,            // E: Full Year
    ];
    if (monthlyArr) {
    ...
    vals.push(ytd != null && ytd !== 0 ? round2(ytd) : null);  // R: YTD
  ...
  // Régularisation sommen (voor Mouvements provisie)
  const regulCodes = ['490-00-631', '493-00-731', '490-00-635', '493-00-719', '493-00-735'];
  let regulYTD = 0;
  ...
  // Provisie-rijen mapping: code → provisie rijen die erna komen (alleen label + YTD waarde)
  type PLProvisionRow = { label: string; ytd: number };
  ...
  if (piscineProvClosing !== 0) {
    piscineProvs.push({ label: `Provision pour location de piscine en ${year}`, ytd: -piscineProvClosing });
  }
  if (piscineProvs.length > 0) provisionsByCode.set('610-00-622', piscineProvs);
  ...
  if (materielProvClosing !== 0) {
    materielProvs.push({ label: `Provision pour achat de matériel en ${year + 1}`, ytd: -materielProvClosing });
  }
  if (materielProvs.length > 0) provisionsByCode.set('601-00-624', materielProvs);
  ...
  for (const gd of sortedGroups) {
    ...
    // Group subtotal row (bold)
    writePLRow(currentRow, gd.group.name, '', groupMonthly, groupYTD, groupBudget, true);
    sheet.getRow(currentRow).height = 13.5;
  ...
  // ============================
  // BOTTOM SECTION: Cash Balance reconciliation (same as Résultats courant)
  // ============================
  if (openingBankBalance !== undefined) {
    writePLRow(currentRow, null, 'Opening Cash Balance', null, round2(openingBankBalance), null);
    currentRow++;

    writePLRow(currentRow, null, `Total Mouvement${year}`, null, totalPL_YTD, null);
    currentRow++;

    writePLRow(currentRow, null, 'Apport Cash Reserve', null, null, null);
    currentRow++;
    ...
    writePLRow(currentRow, null, `Reprise provisions ${year}`, null, round2(-totalRepriseProvisions), null);
    provAdj += round2(-totalRepriseProvisions);
    currentRow++;

    if (piscineProvClosing !== 0) {
      writePLRow(currentRow, null, `Provision pour location de piscine en ${year}`, null, piscineProvClosing, null);
      provAdj += piscineProvClosing;
      currentRow++;
    }

    if (materielProvClosing !== 0) {
      writePLRow(currentRow, null, `Provision pour achat de matériel en ${year + 1}`, null, materielProvClosing, null);
      provAdj += materielProvClosing;
      currentRow++;
    }

    // Mouvements provision
    const mouvProv = round2(regulYTD);
    writePLRow(currentRow, null, `Provision pour Mouvements ${year} affecté à ${year + 1}`, null, mouvProv, null);
    provAdj += mouvProv;
    currentRow++;

    // Balance Théorique
    const balanceTheorique = round2(openingBankBalance + totalPL_YTD + provAdj);
    writePLRow(currentRow, null, `Balance Theorique ${year}`, null, balanceTheorique, null);
    currentRow++;

    // Balance Banque
    if (closingBankBalance !== undefined) {
      writePLRow(currentRow, null, 'Balance Banque', null, round2(closingBankBalance), null);
      currentRow++;
      writePLRow(currentRow, null, 'Difference', null, round2(closingBankBalance - balanceTheorique), null);
    } else {
      writePLRow(currentRow, null, 'Balance Banque', null, null, null);
      currentRow++;
      writePLRow(currentRow, null, 'Difference', null, null, null);
    }
  }
  ...
  addResultatsCourantSheet(workbook, fiscalYear, codeMonthlyMap, fiscalYearStart, groups, openingBankBalance, closingBankBalance, bilanValues);
