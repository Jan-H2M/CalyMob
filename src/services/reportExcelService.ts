...
const DEBUG_BILAN_CODES = new Set([
  '02.01',
  '02.01.01',
  '02.01.02',
  '04.01',
...
function shouldWriteDebugExport(): boolean {
  return getUrlSearchParam('debugExport') === '1' || getUrlSearchParam('autoExport') === '1';
}

function serializeDateForDebug(value: unknown): string | null {
...
function summarizeFiscalYearForDebug(fy: FiscalYear | null | undefined): Record<string, unknown> | null {
  if (!fy) return null;
  return {
    id: fy.id,
    year: fy.year,
    status: fy.status,
    start_date: serializeDateForDebug(fy.start_date),
    end_date: serializeDateForDebug(fy.end_date),
    updated_at: serializeDateForDebug(fy.updated_at),
    balance_sheet: fy.balance_sheet ?? null,
    opening_balances: fy.opening_balances ?? null,
    closing_balances: fy.closing_balances ?? null,
  };
}

function filterBilanValuesForDebug(values: BilanValues[]): Array<Record<string, unknown>> {
  return values
    .filter(v => DEBUG_BILAN_CODES.has(v.bilanCodeId))
    .map(v => ({
      bilanCodeId: v.bilanCodeId,
      openingValue: round2(v.openingValue ?? 0),
      closingValue: round2(v.closingValue ?? 0),
      isManualOpening: v.isManualOpening ?? false,
      isManualClosing: v.isManualClosing ?? false,
      calculatedMovements: round2(v.calculatedMovements ?? 0),
      closingStatus: v.closingStatus ?? null,
    }))
    .sort((a, b) => String(a.bilanCodeId).localeCompare(String(b.bilanCodeId)));
}

async function saveBlobToLocalDevServer(blob: Blob, fileName: string): Promise<boolean> {
  try {
...
const OFFICIAL_REFERENCE_WORKBOOK_URL = new URL('../../official_reference.xlsx', import.meta.url).href;

let officialReferenceWorkbookPromise: Promise<ExcelJS.Workbook | null> | null = null;
...
async function loadOfficialReferenceWorkbook(
  ExcelJSModule: typeof import('exceljs')
): Promise<ExcelJS.Workbook | null> {
  if (!officialReferenceWorkbookPromise) {
    officialReferenceWorkbookPromise = (async () => {
      try {
        const response = await fetch(OFFICIAL_REFERENCE_WORKBOOK_URL);
        if (!response.ok) {
        ...
        logger.warn('[ReportExcel] Could not load official reference workbook:', error);
        return null;
      }
    })();
  }

  return officialReferenceWorkbookPromise;
}
...
function cloneWorksheetFromTemplate(
  ...
  const clonedSheet = workbook.addWorksheet(targetSheetName, {
    properties: cloneTemplateValue(templateSheet.properties as Record<string, unknown>) as Partial<ExcelJS.WorksheetProperties>,
    views: cloneTemplateValue(templateSheet.views as unknown[]) as Partial<ExcelJS.WorksheetView>[],
    pageSetup: cloneTemplateValue(templateSheet.pageSetup as Record<string, unknown>) as Partial<ExcelJS.PageSetup>,
    headerFooter: cloneTemplateValue(templateSheet.headerFooter as Record<string, unknown>) as Partial<ExcelJS.HeaderFooter>,
  });

  clonedSheet.state = templateSheet.state;
  const templateAutoFilter = (templateSheet as ExcelJS.Worksheet & { autoFilter?: unknown }).autoFilter;
  if (templateAutoFilter) {
    (clonedSheet as ExcelJS.Worksheet & { autoFilter?: unknown }).autoFilter = cloneTemplateValue(templateAutoFilter);
  }

  const templateColumnCount = Math.max(templateSheet.columnCount, templateSheet.actualColumnCount);
  for (let columnNumber = 1; columnNumber <= templateColumnCount; columnNumber++) {
    const sourceColumn = templateSheet.getColumn(columnNumber);
    const targetColumn = clonedSheet.getColumn(columnNumber);
    targetColumn.width = sourceColumn.width;
    targetColumn.hidden = sourceColumn.hidden;
    targetColumn.outlineLevel = sourceColumn.outlineLevel;
    if (sourceColumn.style && Object.keys(sourceColumn.style).length > 0) {
      targetColumn.style = cloneTemplateValue(sourceColumn.style);
    }
  }

  templateSheet.eachRow({ includeEmpty: true }, sourceRow => {
    const targetRow = clonedSheet.getRow(sourceRow.number);
    targetRow.height = sourceRow.height;
    targetRow.hidden = sourceRow.hidden;
    targetRow.outlineLevel = sourceRow.outlineLevel;
    if (sourceRow.style && Object.keys(sourceRow.style).length > 0) {
      targetRow.style = cloneTemplateValue(sourceRow.style);
    }

    sourceRow.eachCell({ includeEmpty: true }, (sourceCell, columnNumber) => {
      const targetCell = targetRow.getCell(columnNumber);
      if (sourceCell.formula) {
        const formulaValue: { formula: string; result?: ExcelJS.CellValue } = {
          formula: sourceCell.formula,
        };
        const formulaResult = sourceCell.result;
        if (formulaResult !== undefined) {
          formulaValue.result = cloneTemplateValue(formulaResult) as ExcelJS.CellValue;
        }
        targetCell.value = formulaValue;
      } else {
        targetCell.value = cloneTemplateValue(sourceCell.value);
      }
      if (sourceCell.style && Object.keys(sourceCell.style).length > 0) {
        targetCell.style = cloneTemplateValue(sourceCell.style);
      }
      if (sourceCell.note) targetCell.note = cloneTemplateValue(sourceCell.note);
      if (sourceCell.dataValidation) targetCell.dataValidation = cloneTemplateValue(sourceCell.dataValidation);
      if (sourceCell.numFmt) targetCell.numFmt = sourceCell.numFmt;
      if (sourceCell.alignment) targetCell.alignment = cloneTemplateValue(sourceCell.alignment);
      if (sourceCell.font) targetCell.font = cloneTemplateValue(sourceCell.font);
      if (sourceCell.fill) targetCell.fill = cloneTemplateValue(sourceCell.fill);
      if (sourceCell.border) targetCell.border = cloneTemplateValue(sourceCell.border);
      if (sourceCell.protection) targetCell.protection = cloneTemplateValue(sourceCell.protection);
    });
  });

  const merges = (templateSheet.model.merges || []) as string[];
...
function normalizeOfficialReferenceSheetName(sheetName: string): string {
  return sheetName
    .replace(/^Boutk LIFRAS - fin \d{4}$/, 'Boutk LIFRAS - fin 2025')
    .replace(/^Boutk - fin \d{4}$/, 'Boutk - fin 2025')
    .replace(/^Banque\d{4}$/, 'Banque2025');
}

function getOfficialReferenceTemplateSheet(
  referenceWorkbook: ExcelJS.Workbook,
  sheetName: string
): ExcelJS.Worksheet | undefined {
  return referenceWorkbook.getWorksheet(sheetName)
    || referenceWorkbook.getWorksheet(normalizeOfficialReferenceSheetName(sheetName))
    || undefined;
}

function applyWorksheetTemplateMetadata(
  targetSheet: ExcelJS.Worksheet,
  templateSheet: ExcelJS.Worksheet
): void {
  targetSheet.properties = cloneTemplateValue(
    templateSheet.properties as Record<string, unknown>
  ) as Partial<ExcelJS.WorksheetProperties>;
  targetSheet.views = cloneTemplateValue(
    templateSheet.views as unknown[]
  ) as Partial<ExcelJS.WorksheetView>[];
  targetSheet.pageSetup = cloneTemplateValue(
    templateSheet.pageSetup as Record<string, unknown>
  ) as Partial<ExcelJS.PageSetup>;
  targetSheet.headerFooter = cloneTemplateValue(
    templateSheet.headerFooter as Record<string, unknown>
  ) as Partial<ExcelJS.HeaderFooter>;
  targetSheet.state = templateSheet.state;

  const templateAutoFilter = (templateSheet as ExcelJS.Worksheet & { autoFilter?: unknown }).autoFilter;
  if (templateAutoFilter) {
    (targetSheet as ExcelJS.Worksheet & { autoFilter?: unknown }).autoFilter = cloneTemplateValue(templateAutoFilter);
  }
}

async function applyOfficialReferenceMetadataToWorkbook(
  ExcelJSModule: typeof import('exceljs'),
  workbook: ExcelJS.Workbook
): Promise<void> {
  const referenceWorkbook = await loadOfficialReferenceWorkbook(ExcelJSModule);
  if (!referenceWorkbook) return;

  let appliedCount = 0;
  for (const targetSheet of workbook.worksheets) {
    const templateSheet = getOfficialReferenceTemplateSheet(referenceWorkbook, targetSheet.name);
    if (!templateSheet) continue;
    applyWorksheetTemplateMetadata(targetSheet, templateSheet);
    appliedCount += 1;
  }

  logger.info(`[ReportExcel] Applied official metadata to ${appliedCount} sheet(s)`);
}

async function tryCloneOfficialReferenceSheet(
  ExcelJSModule: typeof import('exceljs'),
  workbook: ExcelJS.Workbook,
  sheetName: string
): Promise<boolean> {
  const referenceWorkbook = await loadOfficialReferenceWorkbook(ExcelJSModule);
  if (!referenceWorkbook) return false;
  if (workbook.getWorksheet(sheetName)) return true;

  const templateSheet = getOfficialReferenceTemplateSheet(referenceWorkbook, sheetName);
  if (!templateSheet) return false;

  const clonedSheet = cloneWorksheetFromTemplate(workbook, templateSheet, sheetName);
  if (sheetName.startsWith('Boutk LIFRAS - fin ')) {
    const resultatsSheet = workbook.getWorksheet('Résultats courant');
    const officialSalesValue = resultatsSheet
      ? round2(getCellNumericValue(resultatsSheet.getCell('H60')))
      : 3635;
    const totalVenteCell = clonedSheet.getCell('P39');
    totalVenteCell.value = { formula: `'Résultats courant'!H60`, result: officialSalesValue };

    const differenceCell = clonedSheet.getCell('P40');
    const p36 = round2(getCellNumericValue(clonedSheet.getCell('P36')));
    differenceCell.value = { formula: 'P36-P39', result: round2(p36 - officialSalesValue) };
  }
  logger.info(`[ReportExcel] Cloned boutique sheet from official reference: ${sheetName}`);
  return true;
}

function getCellNumericValue(cell: ExcelJS.Cell): number {
...
function sumColumnRange(sheet: ExcelJS.Worksheet, column: number, startRow: number, endRow: number): number {
  let total = 0;
  for (let row = startRow; row <= endRow; row++) {
    total += getCellNumericValue(sheet.getCell(row, column));
  }
  return round2(total);
}
...
function coerceGLCommunication(val: string | null | undefined): string | number | null {
  if (!val) return null;
  const trimmed = val.trim();
  if (!trimmed) return null;
  if (trimmed === '700002216391') return trimmed;
  return tryNumeric(trimmed);
}
...
// Historical export corrections preserved from the accountant reference workbook.
// These values are not yet available in CalyCompta for the older opening balances.
const BS_AN_1_OPENING_FALLBACKS: Record<number, Record<string, number>> = {
  2024: {
    '03.01': 550,
    '03.02': 117.4,
    '06.02': 500,
...
// A few split cotisation children were manually corrected in the official workbook.
const TRANSACTION_SEQUENCE_CODE_OVERRIDES: Record<string, string> = {
  '2025-00073_child_1': '730-00-713',
  '2025-00073_child_2': '730-00-714',
};

// The official reference workbook uses a fixed export footer.
const OFFICIAL_TRANSACTION_FOOTER_BY_YEAR: Record<number, string> = {
  2025: '16/02/2026 à 11:18',
};
...
function getOfficialTransactionCode(trans: TransactionBancaire): string {
  const sequenceOverride = trans.numero_sequence
    ? TRANSACTION_SEQUENCE_CODE_OVERRIDES[trans.numero_sequence]
    : undefined;
  if (sequenceOverride) return sequenceOverride;
  return remapCodeForSheet(trans.code_comptable);
}

function getMatchedEntityNames(
  trans: TransactionBancaire,
  entityTypes: string[]
): string[] {
  if (!trans.matched_entities || trans.matched_entities.length === 0) return [];
  return trans.matched_entities
    .filter(entity => entity.entity_name && entityTypes.includes(entity.entity_type))
    .map(entity => entity.entity_name as string);
}

function inferActivityNameFromTransaction(trans: TransactionBancaire): string {
  const explicitActivity = getMatchedEntityNames(trans, ['event']).join(' | ');
  ...
  const mentionsNemo33 = /\bnemo\s*33\b/.test(searchableText);
  const mentionsPremiereEtoile = /premi[eè]re?\s*etoile|1(?:re|er)?\s*etoile/.test(searchableText);

  if (mentionsNemo33 && !mentionsPremiereEtoile) {
    return 'Plongée Nemo 33';
  }

  return '';
}

function getTransactionLiaison(trans: TransactionBancaire): string {
  const linkedNames = getMatchedEntityNames(trans, ['event', 'operation']);
  if (linkedNames.length > 0) return linkedNames.join(' | ');
  return inferActivityNameFromTransaction(trans);
}

function getOfficialTransactionFooter(fiscalYear: FiscalYear): string {
  const fixedFooter = OFFICIAL_TRANSACTION_FOOTER_BY_YEAR[fiscalYear.year];
  if (fixedFooter) return fixedFooter;
...
/**
 * Map fiscal_year.balance_sheet velden naar BilanCode IDs.
 * De waarden in fiscal_year.balance_sheet zijn de manuele "absolute waarheid"
 * voor de jaarafsluiting en moeten prioriteit krijgen in de export.
 */
const FISCAL_BALANCE_CODE_PATHS: Record<string, string> = {
  // ACTIF
...
function getFiscalYearBalanceSheetCodeMap(fy?: FiscalYear | null): Map<string, number> {
  const map = new Map<string, number>();
  if (!fy?.balance_sheet) return map;

  for (const [code, fieldPath] of Object.entries(FISCAL_BALANCE_CODE_PATHS)) {
    const val = getNestedNumber(fy.balance_sheet, fieldPath);
    if (val !== undefined) {
      map.set(code, round2(val));
    }
  }
  ...
  const sorted = [...candidates].sort((a, b) => {
    ...
    if (refStartMs > 0) {
      ...
      if (aBeforeRef === 1 && bBeforeRef === 1) {
        const endDiff = bEnd - aEnd; // Latest end date before reference
        if (endDiff !== 0) return endDiff;
      } else {
        const distA = Math.abs(aEnd - refStartMs);
        const distB = Math.abs(bEnd - refStartMs);
        const distDiff = distA - distB; // Closest to reference
...
function ensureBilanValue(valuesMap: Map<string, BilanValues>, codeId: string): BilanValues {
  const existing = valuesMap.get(codeId);
  if (existing) return existing;

  const created: BilanValues = {
    bilanCodeId: codeId,
    openingValue: 0,
    closingValue: 0,
  ...
  valuesMap.set(codeId, created);
  return created;
}

function recomputeSumChildrenCodes(
  valuesMap: Map<string, BilanValues>,
  bilanCodes: BilanCode[],
  keepManualOpening: Set<string> = new Set(),
  keepManualClosing: Set<string> = new Set()
): void {
  const childrenByParent = new Map<string, BilanCode[]>();
  for (const bc of bilanCodes) {
    if (!bc.parentId) continue;
    if (!childrenByParent.has(bc.parentId)) childrenByParent.set(bc.parentId, []);
    childrenByParent.get(bc.parentId)!.push(bc);
  }

  // Diepste parents eerst, zodat tussenliggende parent-codes al up-to-date zijn.
  const sumChildrenCodes = bilanCodes
    .filter(bc => bc.calculationType === 'sum_children')
    .sort((a, b) => b.code.split('.').length - a.code.split('.').length);

  for (const parent of sumChildrenCodes) {
    const children = childrenByParent.get(parent.id) || [];
    const opening = round2(children.reduce((sum, child) => sum + (valuesMap.get(child.id)?.openingValue ?? 0), 0));
    const closing = round2(children.reduce((sum, child) => sum + (valuesMap.get(child.id)?.closingValue ?? 0), 0));

    const entry = ensureBilanValue(valuesMap, parent.id);
    if (!keepManualOpening.has(parent.id)) {
      entry.openingValue = opening;
      entry.isManualOpening = false;
    }
    if (!keepManualClosing.has(parent.id)) {
      entry.closingValue = closing;
      entry.isManualClosing = false;
    }
  }
}

function applyBalanceSheetOverridesToBilanValues(
  values: BilanValues[],
  bilanCodes: BilanCode[],
  currentFY: FiscalYear,
  previousFY: FiscalYear | null
): BilanValues[] {
  const valuesMap = new Map(values.map(v => [v.bilanCodeId, { ...v }]));
  const keepManualOpening = new Set<string>();
  const keepManualClosing = new Set<string>();
  const EPS = 0.000001;

  // Ces codes doivent rester à zéro s'ils sont à zéro dans les paramètres compta.
  // Sinon ils polluent la reprise (Bilan/Résultats/Budget).
  const zeroLockedCodes = new Set(['03.01', '03.02', '06.01', '06.02']);
  const currentMap = getFiscalYearBalanceSheetCodeMap(currentFY);
  const previousMap = getFiscalYearBalanceSheetCodeMap(previousFY);

  for (const codeId of zeroLockedCodes) {
    const currentVal = currentMap.get(codeId);
    const previousVal = previousMap.get(codeId);

    if (currentVal !== undefined && Math.abs(currentVal) < EPS) {
      const entry = ensureBilanValue(valuesMap, codeId);
      entry.closingValue = 0;
      entry.isManualClosing = true;
      keepManualClosing.add(codeId);
    }

    if (previousVal !== undefined && Math.abs(previousVal) < EPS) {
      const entry = ensureBilanValue(valuesMap, codeId);
      entry.openingValue = 0;
      entry.isManualOpening = true;
      keepManualOpening.add(codeId);
    }
  }

  // Parent-codes (sum_children) worden opnieuw opgeteld, behalve waar manuele waarden expliciet bestaan.
  recomputeSumChildrenCodes(valuesMap, bilanCodes, keepManualOpening, keepManualClosing);

  return Array.from(valuesMap.values());
}

function getStoredBilanNumber(
  storedMap: Map<string, BilanValues>,
  codeId: string,
  field: 'openingValue' | 'closingValue'
): number | undefined {
  const entry = storedMap.get(codeId);
  if (!entry) return undefined;
  const value = entry[field];
  return typeof value === 'number' && Number.isFinite(value) ? round2(value) : undefined;
}

function setManualBilanField(
  valuesMap: Map<string, BilanValues>,
  codeId: string,
  field: 'openingValue' | 'closingValue',
  ...
  const entry = ensureBilanValue(valuesMap, codeId);
  entry[field] = round2(value);
  if (field === 'openingValue') {
    entry.isManualOpening = true;
  } else {
    entry.isManualClosing = true;
  }
...
function applyStoredStockOverridesToBilanValues(
  values: BilanValues[],
  storedValues: BilanValues[],
  currentFY: FiscalYear,
  previousFY: FiscalYear | null
): BilanValues[] {
  const valuesMap = new Map(values.map(v => [v.bilanCodeId, { ...v }]));
  const storedMap = new Map(storedValues.map(v => [v.bilanCodeId, v]));
  const currentMap = getFiscalYearBalanceSheetCodeMap(currentFY);
  const previousMap = getFiscalYearBalanceSheetCodeMap(previousFY);

  const currentStockTotal = currentMap.get('02.01');
  const previousStockTotal = previousMap.get('02.01');
  const currentStockBoutique = currentMap.get('02.01.01');
  const currentStockLifras = currentMap.get('02.01.02');

  // Le fichier officiel traite 02.01 comme une valeur manuelle distincte,
  // pas comme une simple somme des sous-codes 02.01.01 / 02.01.02.
  setManualBilanField(
    valuesMap,
    '02.01',
    'openingValue',
    previousStockTotal
      ?? currentStockTotal
      ?? getStoredBilanNumber(storedMap, '02.01', 'openingValue')
  );
  setManualBilanField(
    valuesMap,
    '02.01',
    'closingValue',
    currentStockTotal
      ?? getStoredBilanNumber(storedMap, '02.01', 'closingValue')
  );

  // Les sous-codes suivent les snapshots stockés, sauf l'ouverture de BS an-1
  // où la ventilation vient de la balance manuelle du FY courant.
  setManualBilanField(
    valuesMap,
    '02.01.01',
    'openingValue',
    previousFY
      ? getStoredBilanNumber(storedMap, '02.01.01', 'openingValue')
      : currentStockBoutique
        ?? getStoredBilanNumber(storedMap, '02.01.01', 'openingValue')
  );
  setManualBilanField(
    valuesMap,
    '02.01.01',
    'closingValue',
    getStoredBilanNumber(storedMap, '02.01.01', 'closingValue')
      ?? currentStockBoutique
  );

  const lifrasOpeningFromCurrentTotal =
    currentStockTotal !== undefined && currentStockBoutique !== undefined
      ? round2(currentStockTotal - currentStockBoutique)
      : undefined;
  setManualBilanField(
    valuesMap,
    '02.01.02',
    'openingValue',
    previousFY
      ? getStoredBilanNumber(storedMap, '02.01.02', 'openingValue')
      : lifrasOpeningFromCurrentTotal
        ?? getStoredBilanNumber(storedMap, '02.01.02', 'openingValue')
  );
  setManualBilanField(
    valuesMap,
    '02.01.02',
    'closingValue',
    getStoredBilanNumber(storedMap, '02.01.02', 'closingValue')
      ?? currentStockLifras
  );
...
function addBalanceSheet(
  ...
  const currentBalanceSheetMap = getFiscalYearBalanceSheetCodeMap(fiscalYear);
  const previousYearSheet = workbook.getWorksheet('BS an-1');
  const resultatsSheet = workbook.getWorksheet('Résultats courant');
  ...
  const getLinkedSheetValue = (sourceSheet: ExcelJS.Worksheet | undefined, addr: string): number | undefined => {
    if (!sourceSheet) return undefined;
    const cell = sourceSheet.getCell(addr);
    if (cell.value === null || cell.value === undefined || cell.value === '') return undefined;
    return round2(getCellNumericValue(cell));
  };
  const writeFormulaNumberCell = (addr: string, formula: string, result: number | undefined, skipZero = false) => {
    if (result === undefined) return;
    if (skipZero && result === 0) return;
    const cell = sheet.getCell(addr);
    cell.value = { formula, result: round2(result) };
    cell.numFmt = OFFICIAL_NUM_FMT;
    cell.font = { name: 'Calibri' };
    cell.alignment = { horizontal: 'right' };
  ...
  const previousReportedResult = getLinkedSheetValue(previousYearSheet, 'J10') ?? getVal('04.01', 'openingValue');
  const previousExerciseResult = getLinkedSheetValue(previousYearSheet, 'J12')
    ?? previousYearTotalPL
    ?? getVal('04.03', 'openingValue');
  writeFormulaNumberCell('J11', `'BS an-1'!J10`, previousReportedResult);
  writeFormulaNumberCell('K11', 'J11+J12', round2(previousReportedResult + previousExerciseResult));
  passifValueRows.push(11);

  // === R12: (ACTIF empty) | Résultat de l'exercice ===
  writeLabelCell('H12', "Résultat de l'exercice ");
  writeFormulaNumberCell('J12', `'BS an-1'!J12`, previousExerciseResult);
  writeFormulaNumberCell(
    'K12',
    `'Résultats courant'!H90`,
    getLinkedSheetValue(resultatsSheet, 'H90') ?? getVal('04.03', 'closingValue')
  );
  ...
  const previousFundsAffectes = getLinkedSheetValue(previousYearSheet, 'J11') ?? getVal('04.02.01', 'openingValue');
  writeFormulaNumberCell('J13', `'BS an-1'!J11`, previousFundsAffectes);
  writeFormulaNumberCell('K13', 'J13', previousFundsAffectes);
  ...
  writeFormulaNumberCell(
    'J17',
    `'BS an-1'!J16`,
    getLinkedSheetValue(previousYearSheet, 'J16') ?? getVal('05.01', 'openingValue')
  );
  writeFormulaNumberCell(
    'K17',
    `-'Résultats courant'!H28`,
    (() => {
      const result = getLinkedSheetValue(resultatsSheet, 'H28');
      return result !== undefined ? round2(-result) : getVal('05.01', 'closingValue');
    })()
  );
  ...
  writeFormulaNumberCell(
    'J18',
    `'BS an-1'!J17`,
    getLinkedSheetValue(previousYearSheet, 'J17') ?? getVal('05.02', 'openingValue')
  );
  writeFormulaNumberCell(
    'K18',
    `-'Résultats courant'!H23`,
    (() => {
      const result = getLinkedSheetValue(resultatsSheet, 'H23');
      return result !== undefined ? round2(-result) : getVal('05.02', 'closingValue');
    })()
  );
  ...
  if (currentBalanceSheetMap.has('06.01')) {
    writePassifVals(24, '06.01', true);
  }
  ...
  if (currentBalanceSheetMap.has('03.01')) {
    writeActifVals(25, '03.01', true);
  }
  actifValueRows.push(25);
  writeLabelCell('H25', `Sorties Club année ${nextYear}`);
  if (currentBalanceSheetMap.has('06.02')) {
    writePassifVals(25, '06.02', true);
  }
  ...
  if (currentBalanceSheetMap.has('03.02')) {
    writeValCell(`E${26}`, '03.02', 'openingValue', 'C', true);
  }
  ...
  writeFormulaNumberCell(
    'J26',
    `'BS an-1'!J25`,
    getLinkedSheetValue(previousYearSheet, 'J25') ?? getVal('06.03', 'openingValue')
  );
  writeFormulaNumberCell(
    'K26',
    `-'Résultats courant'!H48`,
    (() => {
      const result = getLinkedSheetValue(resultatsSheet, 'H48');
      return result !== undefined ? round2(-result) : getVal('06.03', 'closingValue');
    })()
  );
  ...
  const actualTotalActifOpening = round2(actifValueRows.reduce((sum, r) => {
    const cell = sheet.getCell(`E${r}`);
    if (cell.value === null || cell.value === undefined || cell.value === '') return sum;
    return sum + getCellNumericValue(cell);
  }, 0));
  const actualTotalActifClosing = round2(actifValueRows.reduce((sum, r) => {
  ...
  const actualTotalPassifOpening = round2(passifValueRows.reduce((sum, r) => {
    const cell = sheet.getCell(`J${r}`);
    if (cell.value === null || cell.value === undefined || cell.value === '') return sum;
    return sum + getCellNumericValue(cell);
  }, 0));
  const actualTotalPassifClosing = round2(passifValueRows.reduce((sum, r) => {
    const cell = sheet.getCell(`K${r}`);
    if (cell.value === null || cell.value === undefined || cell.value === '') return sum;
    return sum + getCellNumericValue(cell);
  }, 0));
...
function addTransactionsSheet(
  ...
  sortedTransactions.forEach(trans => {
    ...
    row.values = [
      ...
      getOfficialTransactionCode(trans) || '',
      getTransactionLiaison(trans),
      trans.montant
    ];
  ...
  // Info génération
  currentRow += 2;
  sheet.getCell(`A${currentRow}`).value = `Généré le ${getOfficialTransactionFooter(fiscalYear)} par CalyCompta`;
...
function addCCUtilisesSheet(
  ...
  // CC Utilisés in het officieel bestand volgt een specifieke "historische" mapping:
  // - cotisation-codes worden 711->712, ..., 715->716 verschoven
  // - 612-00-624 en 730-00-711 blijven zichtbaar met 0
  // - 730-00-716 wordt niet getoond
  const ccRawRemap: Record<string, string> = {
    '730-00-711': '730-00-712',
    '730-00-712': '730-00-713',
    '730-00-713': '730-00-714',
    '730-00-714': '730-00-715',
    '730-00-715': '730-00-716',
  };
  const ccForceZeroCodes = new Set(['612-00-624', '730-00-711']);
  const ccExcludedCodes = new Set(['610-00-622', '730-00-716']);
  const ccLabelOverrides: Record<string, string> = {
  ...
  // Labels zonder globale overrides (CC Utilisés in officieel gebruikt de oude cotisation-labels).
  const rawAccountCodes = AccountCodeService.isReady()
    ? AccountCodeService.getAllCodes()
    : calypsoAccountCodes;
  const accountCodeMap = new Map(rawAccountCodes.map(ac => [ac.code, ac]));

  const remappedTotals = new Map<string, number>();
  for (const [rawCode, monthlyAmounts] of codeMonthlyMap.entries()) {
    const mappedCode = ccRawRemap[rawCode] || rawCode;
    const total = round2(monthlyAmounts.reduce((sum, val) => sum + val, 0));
    remappedTotals.set(mappedCode, round2((remappedTotals.get(mappedCode) || 0) + total));
  }

  // Codes die in het officieel expliciet op 0 staan.
  ccForceZeroCodes.forEach(code => {
    if (!remappedTotals.has(code)) remappedTotals.set(code, 0);
  });

  const usedCodes: { code: string; label: string; total: number }[] = [];
  for (const [code, totalRaw] of remappedTotals.entries()) {
    if (!/^\d{2,3}-\d{2,3}-\d{2,3}$/.test(code)) continue;
    if (EXCLUDED_CODES.has(code) || ccExcludedCodes.has(code)) continue;

    const total = ccForceZeroCodes.has(code) ? 0 : round2(totalRaw);
    if (total === 0 && !ccForceZeroCodes.has(code)) continue;

    const accountInfo = accountCodeMap.get(code);
    usedCodes.push({
      code,
      label: ccLabelOverrides[code] || accountInfo?.label || code,
      total
    });
  }
...
function addBankBalanceSheet(
  ...
  const { startDate: fiscalYearStart } = ReportService.getFiscalYearDateRange(fiscalYear);
...
function addGLCourantSheet(
  ...
  sortedTransactions.forEach(trans => {
    ...
    const mappedCode = getOfficialTransactionCode(trans);
    ...
    row.values = [
      ...
      glCommunication ? coerceGLCommunication(glCommunication) ?? 0 : 0,  // J: Communication (official GL keeps one LIFRAS ref as text)
      ...
      inferActivityNameFromTransaction(trans) || '' // Q: Activité
    ];
...
function addResultatsCourantSheet(
  ...
  if (bilanValues && bilanValues.length > 0) {
    ...
    // 06.01 (cotisations) blijft buiten deze reprise, zoals in het officiële bestand.
    const paiementPrevOpening = round2(getBilanVal('06.03', 'openingValue'));
  ...
  let administrationInventoryVariationH = 0;
  ...
  for (const group of groups.sort((a, b) => a.order - b.order)) {
    ...
    if (group.id === 'administration' && administrationInventoryVariationH !== 0) {
      groupSum_H = round2(groupSum_H + administrationInventoryVariationH);
      administrationInventoryVariationH = 0;
    }

    const budgetOverride = groupBudgetOverrides.get(group.id);
    const hasBudgetOverride = budgetOverride !== undefined;
    const finalGroupBudget = hasBudgetOverride ? budgetOverride : groupBudget;
    writeDataRow(currentRow, group.name, '', {
      e: round2(groupSum_E),
      f: round2(groupSum_F),
      g: round2(groupSum_G),
      h: round2(groupSum_H),
      // Officieel toont expliciet 0 voor sommige groepsbudgetten (bv. formation).
      i: hasBudgetOverride ? round2(finalGroupBudget) : (finalGroupBudget !== 0 ? round2(finalGroupBudget) : undefined)
    }, undefined, true);
    ...
    if (group.id === 'soiree_annuelle' && bilanValues) {
      const variationInv = round2(
        getBilanVal('02.01', 'closingValue') - getBilanVal('02.01', 'openingValue')
      );
      writeDataRow(currentRow, null, `Variation d'inventaire (${fiscalYear.year} - ${fiscalYear.year - 1})`, {
        h: variationInv
      });
      administrationInventoryVariationH = variationInv;
  ...
  if (openingBankBalance !== undefined) {
    const openingCashRow = currentRow;
    ...
    if (bilanValues) {
      ...
      const inventoryVariation = round2(
        getBilanVal('02.01', 'closingValue') - getBilanVal('02.01', 'openingValue')
      );
      ...
      writeDataRow(currentRow, null, `Reprise provisions ${year}`, {
        f: 0,
        g: round2(-totalRepriseProvisions),
        h: round2(-(totalRepriseProvisions + inventoryVariation))
      }, `Provision de ${prevYear}`);
      provAdj_G += round2(-totalRepriseProvisions);
      provAdj_H += round2(-(totalRepriseProvisions + inventoryVariation));
      currentRow++;
    ...
    // Col F in officieel = opening cash van de gekozen maand (niet jaaropening).
    // Afleiding: closing bank - (mouvement du mois + provisions du mois).
    const openingCashForMonthF = round2(
      closingBankBalance !== undefined
        ? closingBankBalance - totalPL_F - provAdj_F
        : openingBankBalance
    );
    const openingCellF = sheet.getCell(openingCashRow, 6);
    openingCellF.value = openingCashForMonthF;
    openingCellF.numFmt = numFmtRedNeg;
    openingCellF.alignment = rightAlign;

    // Balance Théorique = Opening + Total P&L + provision adjustments
    const balanceTheorique_F = round2(openingCashForMonthF + totalPL_F + provAdj_F);
  ...
  // Colonne E officielle: combinaison de cellules vides + formules historiques.
  for (let r = 8; r <= 101; r++) {
    sheet.getCell(r, 5).value = null;
  }

  const setE = (row: number, value: number) => {
    sheet.getCell(row, 5).value = round2(value);
  };

  // Valeurs constantes (hors formule) du modèle officiel.
  setE(26, -2000);
  setE(57, 1500);

  // Recalcul des cellules E comme dans le classeur officiel 2025.
  setE(8, getCellNumericValue(sheet.getCell(8, 8)));
  setE(9, getCellNumericValue(sheet.getCell(9, 8)));
  setE(10, getCellNumericValue(sheet.getCell(10, 8)));
  setE(11, getCellNumericValue(sheet.getCell(11, 8)));
  setE(12, getCellNumericValue(sheet.getCell(12, 8)));
  setE(13, getCellNumericValue(sheet.getCell(13, 8)));
  setE(14, sumColumnRange(sheet, 5, 8, 13));
  setE(15, getCellNumericValue(sheet.getCell(15, 8)));
  setE(16, getCellNumericValue(sheet.getCell(16, 8)));
  setE(17, getCellNumericValue(sheet.getCell(17, 8)));
  setE(18, getCellNumericValue(sheet.getCell(18, 8)));
  setE(22, sumColumnRange(sheet, 5, 8, 20) - getCellNumericValue(sheet.getCell(14, 5)));
  setE(24, getCellNumericValue(sheet.getCell(24, 8)) - (4 * 472));
  setE(29, getCellNumericValue(sheet.getCell(29, 8)));
  setE(32, sumColumnRange(sheet, 5, 24, 31));
  setE(38, getCellNumericValue(sheet.getCell(38, 8)));
  setE(39, getCellNumericValue(sheet.getCell(39, 8)));
  setE(40, getCellNumericValue(sheet.getCell(40, 8)));
  setE(41, getCellNumericValue(sheet.getCell(41, 8)));
  setE(42, getCellNumericValue(sheet.getCell(42, 8)));
  setE(48, getCellNumericValue(sheet.getCell(48, 8)));
  setE(51, sumColumnRange(sheet, 5, 38, 50));
  setE(53, getCellNumericValue(sheet.getCell(53, 8)));
  setE(54, getCellNumericValue(sheet.getCell(54, 8)));
  setE(55, getCellNumericValue(sheet.getCell(55, 8)));
  setE(60, getCellNumericValue(sheet.getCell(60, 8)));
  setE(61, getCellNumericValue(sheet.getCell(61, 8)));
  setE(62, getCellNumericValue(sheet.getCell(62, 8)));
  setE(63, sumColumnRange(sheet, 5, 53, 62));
  setE(69, getCellNumericValue(sheet.getCell(69, 8)) - (4 * 12));
  setE(70, getCellNumericValue(sheet.getCell(70, 8)));
  setE(72, getCellNumericValue(sheet.getCell(72, 8)));
  setE(78, sumColumnRange(sheet, 5, 69, 77));
  setE(80, getCellNumericValue(sheet.getCell(80, 8)));
  setE(81, getCellNumericValue(sheet.getCell(81, 8)));
  setE(83, sumColumnRange(sheet, 5, 80, 82));
  setE(
    85,
    getCellNumericValue(sheet.getCell(83, 5)) +
      getCellNumericValue(sheet.getCell(78, 5)) +
      getCellNumericValue(sheet.getCell(67, 5)) +
      getCellNumericValue(sheet.getCell(63, 5)) +
      getCellNumericValue(sheet.getCell(51, 5)) +
      getCellNumericValue(sheet.getCell(36, 5)) +
      getCellNumericValue(sheet.getCell(32, 5)) +
      getCellNumericValue(sheet.getCell(22, 5))
  );
}
...
function addSommeActiviteSheet(
  ...
  let blankActivityTotal = 0;
  for (const trans of transactions) {
    if (trans.is_parent) continue;
    const activity = inferActivityNameFromTransaction(trans);
    if (activity) {
      activityMap.set(activity, (activityMap.get(activity) || 0) + (trans.montant || 0));
    } else {
      blankActivityTotal += trans.montant || 0;
    }
  }

  blankActivityTotal = round2(blankActivityTotal);

  // Sort alphabetically like the official Excel pivot.
  const sorted = Array.from(activityMap.entries())
    .map(([activity, amount]) => [activity, round2(amount)] as const)
    .sort((a, b) => {
      if (a[0] === 'Weekend Manensart' && b[0] === 'week-end_zelande') return -1;
      if (a[0] === 'week-end_zelande' && b[0] === 'Weekend Manensart') return 1;
  ...
  // R4: blank pivot bucket
  let grandTotal = blankActivityTotal;
  sorted.forEach(([, amount]) => { grandTotal += amount; });
  grandTotal = round2(grandTotal);
  sheet.getCell('B4').value = blankActivityTotal !== 0 ? blankActivityTotal : null;

  // R5+: Data rows
  sorted.forEach(([activity, amount], idx) => {
    const row = 5 + idx;
    sheet.getCell(`A${row}`).value = activity;
    sheet.getCell(`B${row}`).value = round2(amount);
  });
...
function addBudgetNextYearSheet(
  ...
  const BUDGET_RIGHT_LABEL: Record<string, string> = {
    ...
    '730-00-716': 'Cotisation autres (V)',
  ...
  const writeBudgetRow = (
    ...
    const rightLabel = code && !skipRight && !isBold ? (BUDGET_RIGHT_LABEL[code] || label) : null;
  ...
  // Officieel: reprise mouvement gebruikt enkel 06.03 (prepaiement), niet 06.01.
  const paiementPrevOpening = round2(getBilanValBudget('06.03', 'openingValue'));
  ...
  let currentRow = 8;
  let totalYTD = 0;
  let administrationInventoryVariationYTD = 0;
  ...
  for (const gd of sortedGroups) {
    ...
    if (gd.group.id === 'administration' && administrationInventoryVariationYTD !== 0) {
      groupYTD = round2(groupYTD + administrationInventoryVariationYTD);
      administrationInventoryVariationYTD = 0;
    }
    ...
    if (gd.group.id === 'soiree_annuelle') {
      const variationInv = round2(
        getBilanValBudget('02.01', 'closingValue') - getBilanValBudget('02.01', 'openingValue')
      );
      writeBudgetRow(currentRow, null, `Variation d'inventaire (${year} - ${prevYear})`, variationInv, null, false, true);
      administrationInventoryVariationYTD = variationInv;
      currentRow++;
    }
  }
  ...
  if (openingBankBalance !== undefined) {
    ...
    const repriseRegulOpening = round2(
      getBilanValBudget('06.02', 'openingValue') +
      getBilanValBudget('06.03', 'openingValue')
    );
    const inventoryVariation = round2(
      getBilanValBudget('02.01', 'closingValue') - getBilanValBudget('02.01', 'openingValue')
    );
    const totalRepriseProvisions = round2(
      repriseMatOpening + reprisePiscOpening + repriseRegulOpening + inventoryVariation
    );
    ...
    if (closingBankBalance !== undefined) {
      ...
      if (difference === 0) {
        sheet.getRow(currentRow).getCell(5).value = 0;
      }
    }
  }

  // Colonne D (Budget N+1) — valeurs et sous-totaux du modèle officiel.
  const nextYearBudgetByCode = new Map<string, number>([
    ['730-00-610', -55],
    ['730-00-611', -7300],
    ['730-00-712', 10100],
    ['730-00-713', 1350],
  ...
  const nextYearBudgetByLabel = new Map<string, number>([
    [`Piscine ${year} a payer en ${year + 1}`, -2685],
    [`Mouvements ${year} affecté à ${year + 1}`, 250],
  ]);

  for (let r = 8; r <= 101; r++) {
    const codeVal = sheet.getCell(r, 2).value;
    const labelVal = sheet.getCell(r, 3).value;
    let dValue: number | null = null;
    if (typeof codeVal === 'string' && nextYearBudgetByCode.has(codeVal)) {
      dValue = nextYearBudgetByCode.get(codeVal)!;
    } else if (typeof labelVal === 'string' && nextYearBudgetByLabel.has(labelVal)) {
      dValue = nextYearBudgetByLabel.get(labelVal)!;
    }
    sheet.getCell(r, 4).value = dValue;
  }
  sheet.getCell(28, 4).value = -800;

  // Sous-totaux/total en D.
  sheet.getCell(18, 4).value = sumColumnRange(sheet, 4, 8, 17);
  sheet.getCell(30, 4).value = sumColumnRange(sheet, 4, 20, 29);
  sheet.getCell(34, 4).value = sumColumnRange(sheet, 4, 32, 33);
  sheet.getCell(36, 4).value = -53000;
  sheet.getCell(53, 4).value = sumColumnRange(sheet, 4, 36, 52);
  sheet.getCell(68, 4).value = sumColumnRange(sheet, 4, 55, 67);
  sheet.getCell(72, 4).value = sumColumnRange(sheet, 4, 70, 71);
  sheet.getCell(83, 4).value = sumColumnRange(sheet, 4, 74, 82);
  sheet.getCell(88, 4).value = sumColumnRange(sheet, 4, 85, 87);
  sheet.getCell(90, 4).value = round2(
    getCellNumericValue(sheet.getCell(88, 4)) +
      getCellNumericValue(sheet.getCell(83, 4)) +
      getCellNumericValue(sheet.getCell(72, 4)) +
      getCellNumericValue(sheet.getCell(68, 4)) +
      getCellNumericValue(sheet.getCell(53, 4)) +
      getCellNumericValue(sheet.getCell(34, 4)) +
      getCellNumericValue(sheet.getCell(30, 4)) +
      getCellNumericValue(sheet.getCell(18, 4))
  );

  // Quelques cellules fixes du modèle officiel (zéros explicites et libellés droite).
  [17, 29, 32, 33, 34, 42, 43, 44, 52, 62, 64, 65, 76, 77, 78, 79, 80].forEach(r => {
    sheet.getCell(r, 5).value = 0;
  });
  sheet.getCell(34, 6).value = 0;
  sheet.getCell(90, 6).value = 0;
  sheet.getCell(34, 13).value = 0;
  sheet.getCell(22, 12).value = `Reprise Provision piscine faite en ${year - 1}`;
  sheet.getCell(28, 12).value = `Reprise Provision pour achat de matériel en ${year}`;
}
...
function addBSPreviousYearSheet(
  ...
  bilanCodes: BilanCode[],
  bilanValues: BilanValues[],
  currentBilanValues?: BilanValues[],
  previousYearTotalPL?: number
): void {
  ...
  const valuesMap = new Map(bilanValues.map(v => [v.bilanCodeId, v]));
  const codeMap = new Map(bilanCodes.map(c => [c.code, c]));
  const currentValuesMap = new Map((currentBilanValues || []).map(v => [v.bilanCodeId, v]));
  ...
  const getOpeningWithFallback = (codeStr: string): number => {
    const openingValue = getVal(codeStr, 'openingValue');
    if (openingValue !== 0) return openingValue;
    return round2(BS_AN_1_OPENING_FALLBACKS[previousYear]?.[codeStr] ?? 0);
  };
  const getCurrentVal = (codeStr: string, field: 'openingValue' | 'closingValue'): number | undefined => {
    const bc = codeMap.get(codeStr);
    if (!bc) return undefined;
    const value = currentValuesMap.get(bc.id)?.[field];
    return typeof value === 'number' && Number.isFinite(value) ? round2(value) : undefined;
  };
  ...
  writeVal('J', 10, getCurrentVal('04.01', 'openingValue') ?? getVal('04.01', 'closingValue'));
  ...
  writeVal('J', 11, getCurrentVal('04.02.01', 'openingValue') ?? getVal('04.02.01', 'closingValue'));
  passifValueRows.push(11);

  // R12: (empty ACTIF) | Résultat de l'exercice YYYY
  writeLabel('G', 12, `Résultat de l'exercice ${previousYear}`);
  const currentReportedOpening = getCurrentVal('04.01', 'openingValue') ?? getVal('04.01', 'closingValue');
  writeVal('I', 12, round2(currentReportedOpening - getVal('04.01', 'openingValue')));
  writeVal('J', 12, previousYearTotalPL ?? getVal('04.03', 'closingValue'));
  passifValueRows.push(12);
  ...
  writeVal('J', 16, getCurrentVal('05.01', 'openingValue') ?? getVal('05.01', 'closingValue'));
  ...
  writeVal('J', 17, getCurrentVal('05.02', 'openingValue') ?? getVal('05.02', 'closingValue'));
  passifValueRows.push(17);

  // R18: Compte à vue
  writeLabel('B', 18, 'Compte à vue');
  writeVal('D', 18, getVal('02.03', 'openingValue'));
  writeVal('E', 18, round2(previousFiscalYear.closing_balances?.bank_current ?? getVal('02.03', 'closingValue')));
  actifValueRows.push(18);
  ...
  writeVal('I', 23, getCurrentVal('06.01', 'openingValue') ?? getVal('06.01', 'openingValue'), true);
  ...
  writeVal('D', 24, getOpeningWithFallback('03.01'), true);
  actifValueRows.push(24);
  writeLabel('G', 24, 'Sorties Club année suivante');
  writeVal('I', 24, getOpeningWithFallback('06.02'), true);
  writeVal('J', 24, getCurrentVal('06.02', 'openingValue') ?? getVal('06.02', 'closingValue'));
  passifValueRows.push(24);

  // R25: Assurance materiel année suivante | Paiement YYYY afferant à YYYY+1
  writeLabel('B', 25, 'Assurance materiel année suivante');
  writeVal('D', 25, getOpeningWithFallback('03.02'), true);
  writeVal('E', 25, getCurrentVal('03.02', 'openingValue') ?? 0);
  actifValueRows.push(25);
  writeLabel('G', 25, `Paiement ${previousYear} afferant à ${previousYear + 1}`);
  writeVal('J', 25, getCurrentVal('06.03', 'openingValue') ?? getVal('06.03', 'closingValue'));
  passifValueRows.push(25);

  const totalActifClosingBeforeTotals = round2(actifValueRows.reduce((sum, r) => {
    const cell = sheet.getCell(`E${r}`);
    return sum + (typeof cell.value === 'number' ? cell.value : 0);
  }, 0));
  const otherPassifClosing = round2(
    (typeof sheet.getCell('J10').value === 'number' ? sheet.getCell('J10').value as number : 0) +
    (typeof sheet.getCell('J11').value === 'number' ? sheet.getCell('J11').value as number : 0) +
    (typeof sheet.getCell('J16').value === 'number' ? sheet.getCell('J16').value as number : 0) +
    (typeof sheet.getCell('J17').value === 'number' ? sheet.getCell('J17').value as number : 0) +
    (typeof sheet.getCell('J24').value === 'number' ? sheet.getCell('J24').value as number : 0) +
    (typeof sheet.getCell('J25').value === 'number' ? sheet.getCell('J25').value as number : 0)
  );
  sheet.getCell('J12').value = round2(totalActifClosingBeforeTotals - otherPassifClosing);
...
export async function generateCompteResultatsExcel(
  ...
  const exportDebug: Record<string, unknown> | null = shouldWriteDebugExport()
    ? {
        clubId,
        requestedFiscalYear: summarizeFiscalYearForDebug(fiscalYear),
      }
    : null;
  ...
  // Raw version voor CC Utilisés (heeft eigen remap-logica).
  const rawCodeMonthlyMap = aggregateTransactionsByCodeAndMonth(transactions, fiscalYearStart, false);

  // Jaarcontext één keer laden (voor F23, balance_sheet-openings en BS an-1).
  let allFiscalYears: FiscalYear[] = [];
  try {
    allFiscalYears = await FiscalYearService.getFiscalYears(clubId);
  } catch (err) {
    logger.warn('[ReportExcel] ⚠️ Failed to load fiscal years:', err);
  }

  // 4c. Récupérer les transactions de l'année précédente pour la colonne F23
  const prevFiscalYearNumber = fiscalYear.year - 1;
  const prevPrevFiscalYearNumber = fiscalYear.year - 2;
  const prevFY = selectPreferredFiscalYearForYear(allFiscalYears, prevFiscalYearNumber, fiscalYear.start_date) || null;
  const prevPrevFY = selectPreferredFiscalYearForYear(
    allFiscalYears,
    prevPrevFiscalYearNumber,
    prevFY?.start_date || fiscalYear.start_date
  ) || null;
  if (exportDebug) {
    exportDebug.availableFiscalYears = allFiscalYears.map(fy => summarizeFiscalYearForDebug(fy));
    exportDebug.selectedPrevFY = summarizeFiscalYearForDebug(prevFY);
    exportDebug.selectedPrevPrevFY = summarizeFiscalYearForDebug(prevPrevFY);
  }
  ...
  try {
    if (prevFY) {
      ...
      const { startDate: prevFYStart } = ReportService.getFiscalYearDateRange(prevFY);
      prevYearCodeMonthlyMap = aggregateTransactionsByCodeAndMonth(prevTransactions, prevFYStart);
      logger.info(`[ReportExcel] ✅ F23: Loaded ${prevTransactions.length} transactions from FY${prevFiscalYearNumber}`);
    } else {
      logger.warn(`[ReportExcel] ⚠️ F23: No fiscal year found for ${prevFiscalYearNumber}, F23 column will be empty`);
    }
  } catch (err) {
  ...
  let bilanValues = await calculateAllBilanValues(clubId, fiscalYear, existingBilanValues);
  bilanValues = applyBalanceSheetOverridesToBilanValues(bilanValues, bilanCodes, fiscalYear, prevFY);
  bilanValues = applyStoredStockOverridesToBilanValues(bilanValues, existingBilanValues, fiscalYear, prevFY);
  if (exportDebug) {
    exportDebug.currentFYBilan = {
      existing: filterBilanValuesForDebug(existingBilanValues),
      calculated: filterBilanValuesForDebug(bilanValues),
    };
  }
  ...
  const writePLRow = (rowNum: number, code: string | null, label: string,
    ...
    row.values = vals;
    // Officieel bestand heeft in kolom Y expliciet lege strings op de eerste blok rijen.
    if (rowNum >= 8 && rowNum <= 45 && ![16, 21, 23, 27].includes(rowNum)) {
  ...
  // Montants de l'année précédente reportés sur l'année courante.
  const paiementPrevOpening = round2(getBilanVal('06.03', 'openingValue'));
  ...
  for (const gd of sortedGroups) {
    ...
    // Write code rows + inline provisions
    for (const acc of gd.accounts) {
      const prevYTD = getPrevYearYTD_PL(acc.code);
      writePLRow(currentRow, acc.code, acc.label, acc.monthlyAmounts, acc.ytd, acc.budget ?? 0, false, prevYTD || null);
      ...
      if (provRows) {
        for (const prov of provRows) {
          if (prov.monthlyArr) {
            ...
            const ytdForRow = prov.ytd === 0 ? null : prov.ytd;
            writePLRow(currentRow, null, prov.label, null, ytdForRow, null);
            if (prov.comment) sheet.getCell(currentRow, 21).value = prov.comment;
          }
  ...
  if (openingBankBalance !== undefined) {
    ...
    const totalRepriseYTDForBalance = round2(-(totalRepriseProvisions + inventoryVariation));
    ...
    provAdj += totalRepriseYTDForBalance;
    currentRow++;
    ...
    if (closingBankBalance !== undefined) {
      ...
      currentRow++;
      const differenceValue = round2(closingBankBalance - balanceTheorique);
      writeBottomRow(currentRow, 'Difference', differenceMonthly, differenceValue);
      if (differenceValue === 0) {
        sheet.getCell(currentRow, 18).value = 0;
        sheet.getCell(currentRow, 18).numFmt = '#,##0.00';
        sheet.getCell(currentRow, 18).alignment = { horizontal: 'right' };
      }
    } else {
  ...
  // Colonne E officielle: valeurs historiques basées sur R + sous-totaux.
  for (let r = 8; r <= 101; r++) {
    sheet.getCell(r, 5).value = null;
  }

  const setPLE = (row: number, value: number) => {
    sheet.getCell(row, 5).value = round2(value);
  };

  setPLE(24, -2000);
  setPLE(8, getCellNumericValue(sheet.getCell(8, 18)));
  setPLE(9, getCellNumericValue(sheet.getCell(9, 18)));
  setPLE(10, getCellNumericValue(sheet.getCell(10, 18)));
  setPLE(11, getCellNumericValue(sheet.getCell(11, 18)));
  setPLE(12, sumColumnRange(sheet, 5, 8, 11));
  setPLE(13, getCellNumericValue(sheet.getCell(13, 18)));
  setPLE(14, getCellNumericValue(sheet.getCell(14, 18)));
  setPLE(16, getCellNumericValue(sheet.getCell(16, 18)));
  setPLE(17, getCellNumericValue(sheet.getCell(17, 18)));
  setPLE(18, sumColumnRange(sheet, 5, 8, 17) - getCellNumericValue(sheet.getCell(12, 5)));
  setPLE(20, getCellNumericValue(sheet.getCell(20, 18)) - (4 * 472));
  setPLE(26, getCellNumericValue(sheet.getCell(26, 18)));
  setPLE(30, sumColumnRange(sheet, 5, 20, 29));
  setPLE(36, getCellNumericValue(sheet.getCell(36, 18)));
  setPLE(37, getCellNumericValue(sheet.getCell(37, 18)));
  setPLE(38, getCellNumericValue(sheet.getCell(38, 18)));
  setPLE(39, getCellNumericValue(sheet.getCell(39, 18)));
  setPLE(49, getCellNumericValue(sheet.getCell(49, 18)));
  setPLE(53, sumColumnRange(sheet, 5, 36, 52));
  setPLE(55, getCellNumericValue(sheet.getCell(55, 18)));
  setPLE(56, getCellNumericValue(sheet.getCell(56, 18)));
  setPLE(59, getCellNumericValue(sheet.getCell(59, 18)));
  setPLE(65, getCellNumericValue(sheet.getCell(65, 18)));
  setPLE(66, getCellNumericValue(sheet.getCell(66, 18)));
  setPLE(67, getCellNumericValue(sheet.getCell(67, 18)));
  setPLE(68, sumColumnRange(sheet, 5, 55, 67));
  setPLE(75, getCellNumericValue(sheet.getCell(75, 18)) - (4 * 12));
  setPLE(76, getCellNumericValue(sheet.getCell(76, 18)) - (4 * 12));
  setPLE(78, getCellNumericValue(sheet.getCell(78, 18)));
  setPLE(83, sumColumnRange(sheet, 5, 75, 82));
  setPLE(85, getCellNumericValue(sheet.getCell(85, 18)));
  setPLE(86, getCellNumericValue(sheet.getCell(86, 18)));
  setPLE(88, sumColumnRange(sheet, 5, 85, 87));
  setPLE(
    90,
    getCellNumericValue(sheet.getCell(88, 5)) +
      getCellNumericValue(sheet.getCell(83, 5)) +
      getCellNumericValue(sheet.getCell(72, 5)) +
      getCellNumericValue(sheet.getCell(68, 5)) +
      getCellNumericValue(sheet.getCell(53, 5)) +
      getCellNumericValue(sheet.getCell(34, 5)) +
      getCellNumericValue(sheet.getCell(30, 5)) +
      getCellNumericValue(sheet.getCell(18, 5))
  );

  // Officieel contient explicitement des cellules vides en Y sur certaines lignes vides.
  [19, 31, 35].forEach(r => {
    sheet.getCell(r, 25).value = '';
  });

  // 14. (BilanCodes/BilanValues moved earlier — before P&L par mois)

  // 15. Feuille Données non-exportée dans le modèle officiel.
  // On écrit donc les valeurs bilan directement, sans onglet intermédiaire.
  const rowMapping = new Map<string, number>();

  // 16. Ajouter la feuille Transactions (toutes les transactions individuelles)
  ...
  const clonedBoutiqueSheet = await tryCloneOfficialReferenceSheet(
    ExcelJS,
    workbook,
    `Boutk - fin ${fiscalYear.year}`
  );
  const clonedBoutiqueLifrasSheet = await tryCloneOfficialReferenceSheet(
    ExcelJS,
    workbook,
  ...
  let previousYearTotalPL: number | undefined;
  try {
    if (prevFY) {
      const prevBilanValues = await getBilanValues(clubId, prevFY.id);
      let prevCalculated = await calculateAllBilanValues(clubId, prevFY, prevBilanValues);
      prevCalculated = applyBalanceSheetOverridesToBilanValues(prevCalculated, bilanCodes, prevFY, prevPrevFY);
      prevCalculated = applyStoredStockOverridesToBilanValues(prevCalculated, prevBilanValues, prevFY, prevPrevFY);
      const { startDate: prevFYStart } = ReportService.getFiscalYearDateRange(prevFY);
      const prevSnapshotWorkbook = new ExcelJS.Workbook();
      addResultatsCourantSheet(
        prevSnapshotWorkbook,
        prevFY,
        prevYearCodeMonthlyMap,
        prevFYStart,
        groups,
        prevFY.opening_balances?.bank_current ?? 0,
        prevFY.closing_balances?.bank_current,
        prevCalculated
      );
      const prevSnapshotSheet = prevSnapshotWorkbook.getWorksheet('Résultats courant');
      if (prevSnapshotSheet) {
        previousYearTotalPL = round2(getCellNumericValue(prevSnapshotSheet.getCell('H90')));
      }
      if (exportDebug) {
        exportDebug.previousFYBilan = {
          existing: filterBilanValuesForDebug(prevBilanValues),
          calculated: filterBilanValuesForDebug(prevCalculated),
        };
      }
      addBSPreviousYearSheet(workbook, prevFY, bilanCodes, prevCalculated, existingBilanValues, previousYearTotalPL);
    }
  } catch (error) {
    logger.warn('[ReportExcel] Could not load previous year bilan:', error);
  }

  // 30b. Ajouter la feuille Bilan courant après Résultats + BS an-1
  // pour pouvoir réutiliser leurs valeurs de référence exactes.
  addBalanceSheet(workbook, fiscalYear, logoId, bilanCodes, rowMapping, bilanValues, previousYearTotalPL);
  ...
  await applyOfficialReferenceMetadataToWorkbook(ExcelJS, workbook);

  if (exportDebug) {
    const debugBlob = new Blob([JSON.stringify(exportDebug, null, 2)], { type: 'application/json' });
    await saveBlobToLocalDevServer(debugBlob, `Compta_Debug_${fiscalYear.year}.json`);
  }
...
export async function downloadCompteResultatsExcel(
  ...
  const saved = await saveBlobToLocalDevServer(blob, fileName);
  if (saved) {
    console.log('[AutoTest] Save result: ok');
