...
function validateManifest(manifest, context) {
  ...
  requireThat(native?.verdict === 'approved' && native.sourceCommit === head && native.sourceTree === tree
  ...
  return { sourceCommit: head, sourceTree: tree, version, build, platform, notes, artifactPath, artifactHash };
}
