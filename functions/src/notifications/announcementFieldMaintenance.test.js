...
describe('announcement cursor-v1 field maintenance', () => {
  test('create sets published visibility and creation activity', () => {
    ...
    expect(changes).toEqual({ visibility: 'published' });
  });
  ...
  test('reply advances activity but an older reply does not regress it', async () => {
    ...
    const db = { collection: () => ({ doc: () => ({ collection: () => ({ doc: () => ref }) }) }), runTransaction: async (work) => work({ get: async () => ({ exists: true, data: () => ({ visibility: 'published', unread_activity_at: ts(20), last_activity_at: ts(20), unread_last_reply_at: ts(20), last_reply_at: ts(20) }) }), update: (_ref, data) => updates.push(data) }) };
    await advanceAnnouncementActivity({ db, clubId: 'c', announcementId: 'a', reply: { unread_created_at: ts(10) } });
    await advanceAnnouncementActivity({ db, clubId: 'c', announcementId: 'a', reply: { unread_created_at: ts(30) } });
    expect(updates).toHaveLength(1);
    expect(updates[0].unread_activity_at.toMillis()).toBe(30);
    expect(updates[0].last_activity_at.toMillis()).toBe(30);
    expect(updates[0].unread_last_reply_at.toMillis()).toBe(30);
    expect(updates[0].last_reply_at.toMillis()).toBe(30);
  });
