 ...
 *           and `clubs/{clubId}/operations/{operationId}/palanquees/{palanqueeId}`
...
async function handlePalanqueeSaved(event) {
  ...
  const operationId = after.operation_id || event.params.operationId || null;
  ...
  for (const [memberId, codes] of Object.entries(afterMap)) {
    ...
    for (const code of codes) {
      ...
      const claimRef = db.collection('clubs').doc(clubId).collection('exercise_claims').doc();
  ...
  console.log(`[${FUNCTION_NAME}] palanquee ${palanqueeId}: ${created} draft claims created, ${skipped} skipped`);
}
...
const onPalanqueeSaved = onDocumentWritten(
  ...
  handlePalanqueeSaved,
);

const onOperationPalanqueeSaved = onDocumentWritten(
  {
    region: FUNCTION_REGION,
    document: 'clubs/{clubId}/operations/{operationId}/palanquees/{palanqueeId}',
  },
  handlePalanqueeSaved,
);

module.exports = { onPalanqueeSaved, onOperationPalanqueeSaved };
