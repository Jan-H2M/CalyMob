...
describe('onPoolCheckinCompleted attendee identity', () => {
  test('writes identity fields without overwriting unknown names with null', () => {
    ...
    expect(buildAttendeeIdentityPatch({ member_id: 'member-a' })).toEqual({
      memberId: 'member-a',
    });
  });

  test('uses context.attendee_id as the canonical completion target', async () => {
  ...
  test('falls back to an existing memberId attendee before creating userId doc', async () => {
  ...
  test('falls back to legacy membre_id attendee documents', async () => {
  ...
  test('creates the fallback doc with userId only when no scan document exists', async () => {
    const { db } = makeDb();
